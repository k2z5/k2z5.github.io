...
class fieldSlider extends cmsFormField {

    public $title       = LANG_PARSER_IMAGES.' + Слайдер + LightGallery';
    public $sql         = 'text';
    ...
    public function parse($value){
        ...
        foreach($images as $key => $paths){
            ...
            if(!empty($paths['original']) &&  strtolower(pathinfo($paths['original'], PATHINFO_EXTENSION)) === 'gif'){
                ...
                $html .= '<a title="'.html($title, false).'" class="img-slider" href="'.html_image_src($paths, $this->getOption('size_full'), true).'">'.html_image($paths, $small_preset, $title.' '.$key, ['class' => 'img-thumbnail']).'</a>';
            }
    ...
    $template->addTplCSSName('jquery-owl.carousel');
	$template->addTplJSName(['jquery-owl.carousel']);
	$template->addCSS('templates/'.$template->name.'/lg/css/lightgallery.min.css'); 
    $template->addJS('templates/'.$template->name.'/lg/js/lightgallery-all.min.js');	
    $template->addBottom('<style>#slider-'.$this->getName().'{overflow:hidden;}
#on_top, #main_menu_box.fixed_nav{ z-index:999 !important; } .lg-backdrop {  background-color: rgb(0,0,0,0.8) !important; }</style>');
    $template->addBottom('<script type="text/javascript">
    $(function(){		
   $("#slider-'.$this->getName().'").owlCarousel({
                items:4,
				loop: false,
                rewind : true,
                margin:10,
		...
		$("#slider-'.$this->getName().'").lightGallery({  selector: ".img-slider"}); 
        ...
        return '<div id="slider-'.$this->getName().'" class="owl-carousel">'.$html.'</div>';
